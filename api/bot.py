...
async def save_answer(answer_data: dict):
    try:
        ...
        # Получаем второй лист с данными о пользователях
        sheet = client.open("quiz").get_worksheet(1)

        # Находим строку с данными пользователя по имени
        cell = sheet.find(name)
        if not cell:
            raise HTTPException(status_code=404, detail="Пользователь не найден.")

        row_index = cell.row
        result_column = sheet.col_values(16)  # Колонка с результатами (16-й столбец)

        # Находим столбец, который соответствует текущему вопросу
        if question_id > len(result_column) - 1:
            raise HTTPException(status_code=400, detail="Неверный ID вопроса.")

        # Записываем результат (0 или 1)
        sheet.update_cell(row_index, question_id + 2, is_correct)  # Мы начинаем с 2-го столбца
