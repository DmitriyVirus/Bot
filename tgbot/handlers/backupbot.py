...
def download_repo_zip():
    ...
    # GitHub архив создаёт папку с именем repo-main
...
def upload_to_gdrive(archive_name):
    ...
    # Создаём временный JSON-файл сервисного аккаунта
    with tempfile.NamedTemporaryFile(mode="w", suffix=".json", delete=False) as f:
        f.write(creds_json)
        json_path = f.name

    # Настройка GoogleAuth через JSON-файл сервисного аккаунта
    gauth = GoogleAuth()
    gauth.LoadServiceConfigFile(json_path)  # <-- рабочий способ
    gauth.ServiceAuth()                     # авторизация
