...
def filter_participants(text: str):
    # Обновленное регулярное выражение для удаления ненужных фрагментов текста, включая точки
    excluded_text = r'Я жду\.\.\.\s*Участвуют \d+ человек\(а\):\s*|Когда|соберемся|\d+|\s*,\s*|\.+'
